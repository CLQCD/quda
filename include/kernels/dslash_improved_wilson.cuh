...
namespace quda
{
  ...
  struct ImprovedWilsonArg : DslashArg<Float, nDim> {
    ...
    ImprovedWilsonArg(ColorSpinorField &out, const ColorSpinorField &in, const GaugeField &U, const GaugeField &L,
      ...
      improve(improve)
    {
      ...
      checkPrecision(out, in, x, U, L); // check all precisions match
      checkLocation(out, in, x, U, L);  // check all locations match
  ...
  */
  ...
  __device__ __host__ inline void applyImprovedWilson(Vector &out, const Arg &arg, Coord &coord, int parity, int idx, int thread_dim, bool &active)
  {

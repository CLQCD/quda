...
#include <kernels/dslash_improved_wilson_clover_preconditioned.cuh>
...
namespace quda
{
  ...
  template <typename Arg> class ImprovedWilsonCloverPreconditioned : public Dslash<improvedWilsonCloverPreconditioned, Arg>
  {
    using Dslash = Dslash<improvedWilsonCloverPreconditioned, Arg>;
    using Dslash::arg;
    using Dslash::in;

  public:
    ImprovedWilsonCloverPreconditioned(Arg &arg, const ColorSpinorField &out, const ColorSpinorField &in) : Dslash(arg, out, in)
    {
    }
  ...
  template <typename Float, int nColor, QudaReconstructType recon> struct ImprovedWilsonCloverPreconditionedApply {
                                                   ...
                                                   TimeProfile &profile)
    {
      ...
      ImprovedWilsonCloverArg<Float, nColor, nDim, recon> arg(out, in, U, L, A, a, x, improve, parity, dagger, comm_override);
      ImprovedWilsonCloverPreconditioned<decltype(arg)> wilson(arg, out, in);
